...
|---|---|---|---|---|---|
| `Shipped.cfg` — the tree as it stands, `SYMMETRY` on, firmware constants | **GREEN** | 986 836 197 | 77 563 872 | 58 | 1864 s |
| `AlwaysUv.cfg` — AS-AUTH-2's other arm: the build that ships it | **GREEN** | 273 124 081 | 23 521 512 | 51 | 508 s |
| `Historical_E76.cfg` — the seed-lead taken back out | `RED: NoUnmanageableCredential` | 1 868 105 | 218 650 | 13 | 5 s |
| `Historical_E77.cfg` — the grant back in phase 2 **and** the consumer fix out | `RED: NoAccessibleSecretWithoutGate` | 1 773 580 | 206 059 | 13 | 5 s |
| 30 × `Mut_*.cfg` — mutant against the whole invariant set | all 30 RED | 89 – 3 384 970 | 40 – 378 649 | 4 – 14 | 0 s – 8 s |
| 33 × `Solo_*.cfg` — the same mutants against their **own** target only | all 33 RED | 89 – 5 617 513 | 40 – 607 193 | 4 – 15 | 0 s – 12 s |
| 3 × `SoloClause_*.cfg` — and against **one clause** of it | all 3 RED | 23 884 – 20 479 961 | 5 469 – 2 063 764 | 8 – 18 | 1 s – 39 s |
| `Fairness.cfg` — `ENABLED OpAdvances => ~Idle`, at the liveness constants | **GREEN** | 105 147 241 | 10 720 348 | 46 | 155 s |
...
| 28 × `SeamMut_*.cfg` / `SeamSolo_*.cfg` | all 28 RED | 77 – 2 030 | 27 – 223 | 4 – 7 | 0 s – 1 s |
| `Store.cfg` — the third module: the flash layer | **GREEN** | 4 185 | 364 | 6 | 1 s |
| 17 × `StoreMut_*.cfg` / `StoreSolo_*.cfg` | all 17 RED | 55 – 221 | 24 – 77 | 3 – 4 | 0 s – 1 s |
...
| 14 × `PolicyMut_*.cfg` / `PolicySolo_*.cfg` | all 14 RED | 33 – 680 | 14 – 126 | 3 – 5 | 0 s – 1 s |
...
| 4 × `TokenRefinement*.cfg` — phase 5: native B -> A state refinement | 2 GREEN, 2 RED | 2 – 34 038 747 | 2 – 3 472 034 | 2 – 44 | 1 s – 207 s |
| `Liveness.cfg` — the three temporal properties, at the heap `floors.txt` gives it | **GREEN** | 105 147 241 | 10 720 348 | 46 | 2020 s |
| 3 × `LiveMut_*.cfg` — one mutant per property | all 3 RED | 494 195 – 626 575 | 73 631 – 94 408 | — | 4 s |
...
<!-- Generated by scripts/run_count_gate.py --write; do not edit. -->
**Every row above is from one run on 2026-08-29**: `./run-tlc.sh safety` and
...
configuration. 198 safety rows in **3220 s** and 4 liveness rows in **2032 s**;
22 GREEN, 180 RED, and not one row missed what `floors.txt` requires of it —
...
The live roster is **72 entries: 67 executable patches killed, five unreachable
...
it is **GREEN over 10 720 348 distinct states in 2020 s**, in the same matrix
