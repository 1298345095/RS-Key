...
| `crates/rsk-fido/src/reset.rs:126` `>` → `>=` | conformance — the runaway valve trips one delete early. Driven in all four applets and green in all four: it differs from `>` only where `deleted` lands EXACTLY on the budget, which the batch below rules out | recorded |
