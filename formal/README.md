...
the permissive state (`reset.rs:187-208`), so what a torn wipe can do is
...
| `BugWarmResetReopensWindow` | `reset.rs:216` `!warm_boot` | `NoAuthorizationBypass` | 126 states |
...
| **Fail-safe direction** | `ctaphid.rs:431` `>` → `>=` refuses an exactly-maximum message: stricter, so `NoBufferOverrun` still holds. `fs.rs:175` and `fs.rs:223` `\|=` → `&=` clear *decided* bits, which sends more reads to the reliable backend |
| **Model-blind** | the dynamic-file registry in `scan` (`fs.rs:228` and `fs.rs:231`, three mutants), `try_has_data`'s zero-length test (`fs.rs:319`), `factory_wipe`'s 64-key batch bound (`fs.rs:441`), the registry retain in `delete` (`fs.rs:541`), and **`meta_delete`'s fault guard (`fs.rs:744`)** |
...
sibling `Fs::meta_delete` has the identical guard at `fs.rs:746`, and **nothing
...
`known_absent` and rebuilds the blob from empty (`fs.rs:707`), so the records go
...
`crates/rsk-piv/src/lib.rs:1312` `&&` → `||` is the sharpest thing this pass has
...
- **`PgpSetPwStatus`'s `held["pw3"]`.** PUT DATA `0xC4` is the only writer of the
  ...
  administrative write gated on PW3 (`crates/rsk-openpgp/src/putdata.rs:244-247`,
...
`sweep` batches whatever `for_each_key` yields, and `fs.rs:322-325` documents
...
|---|---|---|---|---|---|
| `Shipped.cfg` — the tree as it stands, `SYMMETRY` on, firmware constants | **GREEN** | 986 836 197 | 77 563 872 | 58 | 1869 s |
| `AlwaysUv.cfg` — AS-AUTH-2's other arm: the build that ships it | **GREEN** | 273 124 081 | 23 521 512 | 51 | 513 s |
| `Historical_E76.cfg` — the seed-lead taken back out | `RED: NoUnmanageableCredential` | 1 875 114 | 219 667 | 13 | 5 s |
| `Historical_E77.cfg` — the grant back in phase 2 **and** the consumer fix out | `RED: NoAccessibleSecretWithoutGate` | 1 784 803 | 207 245 | 13 | 5 s |
| 30 × `Mut_*.cfg` — mutant against the whole invariant set | all 30 RED | 89 – 3 378 394 | 40 – 377 779 | 4 – 14 | 0 s – 8 s |
| 33 × `Solo_*.cfg` — the same mutants against their **own** target only | all 33 RED | 89 – 5 360 582 | 39 – 584 128 | 4 – 15 | 0 s – 12 s |
| 3 × `SoloClause_*.cfg` — and against **one clause** of it | all 3 RED | 24 937 – 20 230 310 | 5 668 – 2 038 232 | 8 – 18 | 1 s – 40 s |
| `Fairness.cfg` — `ENABLED OpAdvances => ~Idle`, at the liveness constants | **GREEN** | 105 147 241 | 10 720 348 | 46 | 161 s |
| `FairMut_*.cfg` — E160 verbatim | `RED: OpAdvancesIsOneActivity` | 72 | 39 | 4 | 1 s |
| `Seams.cfg` — the second module: the applet seams | **GREEN** | 6 045 | 410 | 11 | 0 s |
| 28 × `SeamMut_*.cfg` / `SeamSolo_*.cfg` | all 28 RED | 77 – 1 825 | 27 – 211 | 4 – 7 | 0 s – 1 s |
| `Store.cfg` — the third module: the flash layer | **GREEN** | 4 185 | 364 | 7 | 1 s |
| 14 × `StoreMut_*.cfg` / `StoreSolo_*.cfg` | all 14 RED | 55 – 195 | 24 – 66 | 3 – 4 | 0 s – 1 s |
...
| 6 × `LatMut_*.cfg` / `LatSolo_*.cfg` | all 6 RED | 2 – 83 | 2 – 31 | 2 – 4 | 0 s – 1 s |
| `Policies.cfg` — all four applets' stateful operation policies | **GREEN** | 45 253 | 2 268 | 14 | 1 s |
| 14 × `PolicyMut_*.cfg` / `PolicySolo_*.cfg` | all 14 RED | 33 – 636 | 14 – 116 | 3 – 5 | 0 s – 1 s |
| `Admin.cfg` — the fifth module: the administrative surface | **GREEN** | 101 | 8 | 3 | 1 s |
...
| 9 × `BootMut_*.cfg` / `BootSolo_*.cfg` / `BootCarryMut_*.cfg` | all 9 RED | 2 – 63 | 2 – 26 | 2 – 5 | 0 s – 1 s |
| `BootInduction.cfg` — `IndInv /\ Next => IndInv'` from ANY admitted state | **GREEN** | 180 | 48 | 1 | 1 s |
| `BootInductionMut_*.cfg` | `RED: MarkerNeverLies` | 101 | 50 | 2 | 1 s |
| `Transport.cfg` — the eighth module: the CTAPHID reassembler | **GREEN** | 127 | 13 | 4 | 0 s |
| 6 × `TransMut_*.cfg` / `TransSolo_*.cfg` | all 6 RED | 28 – 46 | 8 – 11 | 3 – 4 | 0 s – 1 s |
...
| `TraceSecurity.cfg` — raw C-state --beta--> B and alpha == gamma(B) | **GREEN** | 77 | 75 | 75 | 1 s |
| 8 × `TraceSecurityBad*.cfg` — the shifts the replay must refuse, one control among them | 1 GREEN, 7 RED | 11 – 77 | 10 – 75 | 10 – 75 | 0 s – 1 s |
| 4 × `TokenGate*.cfg` — tier A's requirement half, its oracle, their disagreement, and one forbidden edge | 2 GREEN, 2 RED | — | — | — | 1 s |
| 4 × `TokenRefinement*.cfg` — phase 5: native B -> A state refinement | 2 GREEN, 2 RED | 2 – 34 038 747 | 2 – 3 472 034 | 2 – 44 | 1 s – 206 s |
| `Liveness.cfg` — the three temporal properties, at the heap `floors.txt` gives it | **GREEN** | 105 147 241 | 10 720 348 | 46 | 2106 s |
| 3 × `LiveMut_*.cfg` — one mutant per property | all 3 RED | 513 071 – 625 807 | 75 430 – 94 131 | — | 4 s |
...
<!-- Generated by scripts/run_count_gate.py --write; do not edit. -->
**Every row above is from one run on 2026-08-27**: `./run-tlc.sh safety` and
...
configuration. 195 safety rows in **3225 s** and 4 liveness rows in **2118 s**;
22 GREEN, 177 RED, and not one row missed what `floors.txt` requires of it —
...
|---|---|---|
| `NoStatusOutsideItsSelection` | An applet holds a security status only while it is the **selected** applet. Structural — it reads straight out of the state | `crates/rsk-sdk/src/applet.rs:374-390` (the one place that decides what a selection does to the applet that was current) · `crates/rsk-piv/src/lib.rs:199-203` · `crates/rsk-openpgp/src/pin.rs:67-80` · `crates/rsk-oath/src/lib.rs:1200-1204` · `crates/rsk-device/src/ccid.rs:354-369` (the ICC power transition) |
| `NoStatusAfterARefusedAuth` | A reference whose authentication was just refused is not authenticated | `crates/rsk-piv/src/lib.rs:183-186` · `crates/rsk-openpgp/src/pin.rs:176-188` · `crates/rsk-oath/src/lib.rs:1149-1150` |
...
| OATH OTP-PIN `CHANGE` | **both** flags (`crates/rsk-oath/src/lib.rs:1149-1150`) | `aa47867` — before it the whole retry budget could be burned through the door that did not close | `BugFailedChangeKeepsStatus` |
| OATH access-code `VALIDATE` | **nothing** — the standing unlock survives (`crates/rsk-oath/src/lib.rs:529-531`) | a MAC challenge-response has no retry counter for a refusal to protect; a YubiKey 5.7.4 measured keeping it from a genuinely locked applet | `BugRefusedValidateDropsUnlock`, RED in 46 |
...
| Mutation switch | Rebuilds | Target invariant | Caught in |
|---|---|---|---|
| `BugDeleteValueBeforeMeta` | `fs.rs:535-537` — the two backend writes reversed, so a torn delete leaves value-gone-meta-alive (`delete_landed`) | `NoOrphanedMetadata` | 54 states |
...
| `BugDeleteHidesFaultedDrop` | the shipped tree before `fs.rs:542` — a faulted `meta_delete` swallowed, so the caller hears `Ok` about a record standing over a value that is gone | `NoSilentOrphan` | 61 states |
...
| `BugTruncatedScanDecidesAll` | `fs.rs:241-243` — `scan` deciding the whole FID space after a *truncated* walk, so a missed live key reads absent | `NoFalseAbsent` | 24 states |
...
| Mutation switch | Removes | Target invariant | Caught in |
|---|---|---|---|
| `BugUseWhenBlocked` | the `left == 0 => PIN_BLOCKED` floor (`crates/rsk-piv/src/lib.rs:1281-1283` / `crates/rsk-openpgp/src/pin.rs:218-220`), which guards a direct verify AND a recovery reference | `NoAuthWhenBlocked` | 30 states |
| `BugWrongDoesNotSpend` | the decrement that IS the gate (`crates/rsk-piv/src/lib.rs:1299` / `crates/rsk-openpgp/src/pin.rs:125`) | `WrongAttemptIsCharged` | 2 states |
| `BugRecoveryWithoutSecret` | the recovery secret verified before the refill (`crates/rsk-piv/src/lib.rs:1432` / `crates/rsk-openpgp/src/pin.rs:793`) | `BudgetRisesOnlyWithItsSecret` | 9 states |
...
long as it existed. `now_ms` is what separates them (`reset.rs:216`) and
...
empty (`fs.rs:707`), losing `b`. This is SEC-STORE-004's damage arriving from a
...
the `generated TLC configs` row). 200 of the 201 `.cfg` files open with
...
|---|---|---|
| the tree as it stands | `200 generated configuration(s) reproduce byte-for-byte, 1 hand-written` | 0 |
...
the ONE hand-written file, never of the 200 — rewrite the generator's header,
...
| Mutation | What the row said | Exit |
|---|---|---|
| the tree as it stands | `200 configuration(s) held to 60 entries (25 wildcard families covering 165), 6 ratchets, 1 exempt` | 0 |
...
| `SEC-FIDO-006A` | `ResetKeepsThePinGate` | BOUNDED | `RSKeySecurityState` | — | 0 | 1 | 1 | 1 | 1 | 1 |
| `SEC-FIDO-006B` | `ResetKeepsTheAlwaysUvGate` | BOUNDED | `RSKeySecurityState` | — | 0 | 1 | 1 | 1 | 1 | 1 |
| `SEC-FIDO-006C` | `ResetKeepsTheBackupSeal` | BOUNDED | `RSKeySecurityState` | — | 0 | 1 | 0 | 1 | 1 | 1 |
| `SEC-FIDO-007` | `RamNeverOutlivesFlashSeed` | MODELLED-ONLY | `RSKeySecurityState` | — | 1 | 1 | 1 | 0 | 0 | 0 |
...
- **The order within a sweep phase is arbitrary.** `for_each_key` yields in
  flash-ring order (`fs.rs:322-325`), which is *a* fixed order per device state,
...
it is **GREEN over 10 720 348 distinct states in 2106 s**, in the same matrix
