...
|---|---|---|---|---|---|
| `Shipped.cfg` — the tree as it stands, `SYMMETRY` on, firmware constants | **GREEN** | 986 836 197 | 77 563 872 | 58 | 1938 s |
| `AlwaysUv.cfg` — AS-AUTH-2's other arm: the build that ships it | **GREEN** | 273 124 081 | 23 521 512 | 51 | 535 s |
| `PermWide.cfg` — `WidePerms`'s other arm: all sixteen permission subsets | **GREEN** | 344 527 413 | 21 456 020 | 45 | 561 s |
| 2 × `PermWideMut_*.cfg` — and what says that arm's observers can fail | all 2 RED | 3 921 – 146 075 | 1 286 – 17 158 | 7 – 9 | 1 s |
| `ForceChange.cfg` — `ForceChangeModelled`'s other arm: EF_MINPINLEN[1], the forced-PIN-change gate | **GREEN** | 106 956 959 | 10 765 176 | 46 | 178 s |
| `Historical_E76.cfg` — the seed-lead taken back out | **GREEN** | 986 872 269 | 77 563 872 | 58 | 1927 s |
| `Historical_E77.cfg` — the grant back in phase 2 **and** the consumer fix out | `RED: NoAccessibleSecretWithoutGate` | 1 822 749 | 214 790 | 13 | 5 s |
| 31 × `Mut_*.cfg` — mutant against the whole invariant set | all 31 RED | 89 – 3 381 284 | 40 – 378 205 | 4 – 14 | 0 s – 8 s |
| 34 × `Solo_*.cfg` — the same mutants against their **own** target only | all 34 RED | 89 – 6 093 061 | 40 – 664 622 | 4 – 15 | 1 s – 13 s |
| 3 × `SoloClause_*.cfg` — and against **one clause** of it | all 3 RED | 23 357 – 20 473 367 | 5 346 – 2 062 588 | 8 – 18 | 1 s – 40 s |
| `Fairness.cfg` — `ENABLED OpAdvances => ~Idle`, at the liveness constants | **GREEN** | 105 147 241 | 10 720 348 | 46 | 162 s |
...
| 28 × `SeamMut_*.cfg` / `SeamSolo_*.cfg` | all 28 RED | 77 – 1 818 | 27 – 211 | 4 – 7 | 0 s – 1 s |
...
| 14 × `PolicyMut_*.cfg` / `PolicySolo_*.cfg` | all 14 RED | 33 – 674 | 14 – 124 | 3 – 5 | 0 s – 1 s |
...
| 4 × `TokenRefinement*.cfg` — phase 5: native B -> A state refinement | 2 GREEN, 2 RED | 2 – 34 038 747 | 2 – 3 472 034 | 2 – 44 | 1 s – 219 s |
| `Liveness.cfg` — the three temporal properties, at the heap `floors.txt` gives it | **GREEN** | 105 147 241 | 10 720 348 | 46 | 1906 s |
| 3 × `LiveMut_*.cfg` — one mutant per property | all 3 RED | 487 544 – 601 920 | 72 885 – 94 216 | — | 4 s |
...
configuration. 204 safety rows in **6005 s** and 4 liveness rows in **1918 s**;
25 GREEN, 183 RED, and not one row missed what `floors.txt` requires of it —
...
it is **GREEN over 10 720 348 distinct states in 1906 s**, in the same matrix
