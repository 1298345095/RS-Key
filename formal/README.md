...
|---|---|---|---|---|---|
| `Shipped.cfg` — the tree as it stands, `SYMMETRY` on, firmware constants | **GREEN** | 986 836 197 | 77 563 872 | 58 | 1971 s |
| `AlwaysUv.cfg` — AS-AUTH-2's other arm: the build that ships it | **GREEN** | 273 124 081 | 23 521 512 | 51 | 521 s |
| `PermWide.cfg` — `WidePerms`'s other arm: all sixteen permission subsets | **GREEN** | 344 527 413 | 21 456 020 | 45 | 542 s |
| `Historical_E76.cfg` — the seed-lead taken back out | `RED: NoUnmanageableCredential` | 1 875 166 | 219 700 | 13 | 5 s |
| `Historical_E77.cfg` — the grant back in phase 2 **and** the consumer fix out | `RED: NoAccessibleSecretWithoutGate` | 1 695 928 | 198 446 | 13 | 4 s |
| 30 × `Mut_*.cfg` — mutant against the whole invariant set | all 30 RED | 89 – 3 375 244 | 40 – 377 405 | 4 – 14 | 0 s – 8 s |
| 33 × `Solo_*.cfg` — the same mutants against their **own** target only | all 33 RED | 89 – 5 618 025 | 40 – 607 295 | 4 – 15 | 0 s – 12 s |
| 3 × `SoloClause_*.cfg` — and against **one clause** of it | all 3 RED | 23 472 – 20 479 941 | 5 357 – 2 063 763 | 8 – 18 | 1 s – 41 s |
| `Fairness.cfg` — `ENABLED OpAdvances => ~Idle`, at the liveness constants | **GREEN** | 105 147 241 | 10 720 348 | 46 | 160 s |
...
| 28 × `SeamMut_*.cfg` / `SeamSolo_*.cfg` | all 28 RED | 77 – 1 953 | 27 – 218 | 4 – 7 | 0 s – 1 s |
...
| 14 × `PolicyMut_*.cfg` / `PolicySolo_*.cfg` | all 14 RED | 33 – 658 | 14 – 120 | 3 – 5 | 0 s – 1 s |
...
| 9 × `BootMut_*.cfg` / `BootSolo_*.cfg` / `BootCarryMut_*.cfg` | all 9 RED | 2 – 65 | 2 – 26 | 2 – 5 | 0 s – 1 s |
...
| `Liveness.cfg` — the three temporal properties, at the heap `floors.txt` gives it | **GREEN** | 105 147 241 | 10 720 348 | 46 | 2101 s |
| 3 × `LiveMut_*.cfg` — one mutant per property | all 3 RED | 481 348 – 613 831 | 72 252 – 94 840 | — | 3 s – 5 s |
...
configuration. 199 safety rows in **3878 s** and 4 liveness rows in **2113 s**;
23 GREEN, 180 RED, and not one row missed what `floors.txt` requires of it —
...
it is **GREEN over 10 720 348 distinct states in 2101 s**, in the same matrix
