...
| `crates/rsk-openpgp/src/pin.rs:882` guard → `true` | effectively equivalent — an empty `EF_RC` yields `rc_len = 0`, and the `check_pin` below re-reads `EF_RC` and refuses on its own guard | recorded |
...
| `BugRecoveryWithoutSecret` | the recovery secret verified before the refill (`crates/rsk-piv/src/lib.rs:1447` / `crates/rsk-openpgp/src/pin.rs:885`) | `BudgetRisesOnlyWithItsSecret` | 9 states |
