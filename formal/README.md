...
|---|---|---|---|---|---|
| `Shipped.cfg` — the tree as it stands, `SYMMETRY` on, firmware constants | **GREEN** | 1 401 505 939 | 108 618 956 | 58 | 2833 s |
| `AlwaysUv.cfg` — AS-AUTH-2's other arm: the build that ships it | **GREEN** | 385 191 607 | 31 451 172 | 51 | 744 s |
| `PermWide.cfg` — `WidePerms`'s other arm: all sixteen permission subsets | **GREEN** | 497 332 297 | 29 185 896 | 45 | 802 s |
| 2 × `PermWideMut_*.cfg` — and what says that arm's observers can fail | all 2 RED | 4 022 – 136 061 | 1 309 – 16 047 | 7 – 9 | 1 s |
| `ForceChange.cfg` — `ForceChangeModelled`'s other arm: EF_MINPINLEN[1], the forced-PIN-change gate | **GREEN** | 156 123 019 | 14 853 324 | 46 | 256 s |
| `Historical_E76.cfg` — the seed-lead taken back out | **GREEN** | 1 401 567 499 | 108 618 956 | 58 | 2814 s |
| `Historical_E77.cfg` — the grant back in phase 2 **and** the consumer fix out | `RED: NoAccessibleSecretWithoutGate` | 2 167 831 | 241 088 | 13 | 6 s |
| 31 × `Mut_*.cfg` — mutant against the whole invariant set | all 31 RED | 89 – 4 666 690 | 40 – 491 411 | 4 – 14 | 0 s – 11 s |
| 34 × `Solo_*.cfg` — the same mutants against their **own** target only | all 34 RED | 89 – 6 100 085 | 40 – 651 876 | 4 – 15 | 1 s – 13 s |
| 3 × `SoloClause_*.cfg` — and against **one clause** of it | all 3 RED | 28 727 – 29 218 406 | 6 430 – 2 789 498 | 8 – 18 | 1 s – 57 s |
...
| `Store.cfg` — the third module: the flash layer | **GREEN** | 4 185 | 364 | 7 | 0 s |
...
| 6 × `LatMut_*.cfg` / `LatSolo_*.cfg` | all 6 RED | 2 – 73 | 2 – 31 | 2 – 4 | 0 s – 1 s |
| `Policies.cfg` — all four applets' stateful operation policies | **GREEN** | 9 200 521 | 331 776 | 19 | 6 s |
| 20 × `PolicyMut_*.cfg` / `PolicySolo_*.cfg` | all 20 RED | 39 – 14 233 | 16 – 2 029 | 3 – 6 | 0 s – 1 s |
...
| 2 × `Historical_Boot*.cfg` — the write/re-arm order the tree ships, and the one it does not | 1 GREEN, 1 RED | 31 – 141 | 16 – 38 | 4 – 7 | 0 s – 1 s |
...
| 4 × `TokenRefinement*.cfg` — phase 5: native B -> A state refinement | 2 GREEN, 2 RED | 2 – 48 737 609 | 2 – 4 611 484 | 2 – 44 | 1 s – 302 s |
| `Liveness.cfg` — the three temporal properties, at the heap `floors.txt` gives it | **GREEN** | 151 264 817 | 14 514 424 | 46 | 2554 s |
| 3 × `LiveMut_*.cfg` — one mutant per property | all 3 RED | 510 311 – 640 837 | 78 388 – 93 064 | — | 4 s |
...
<!-- Generated by scripts/run_count_gate.py --write; do not edit. -->
**Every row above is from one run on 2026-09-18**: `./run-tlc.sh safety` and
...
configuration. 224 safety rows in **8637 s** and 4 liveness rows in **2566 s**;
...
it is **GREEN over 14 514 424 distinct states in 2554 s**, in the same matrix
