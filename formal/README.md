...
|---|---|---|---|---|---|
| `Shipped.cfg` — the tree as it stands, `SYMMETRY` on, firmware constants | **GREEN** | 986 836 197 | 77 563 872 | 58 | 1880 s |
| `AlwaysUv.cfg` — AS-AUTH-2's other arm: the build that ships it | **GREEN** | 273 124 081 | 23 521 512 | 51 | 512 s |
| `Historical_E76.cfg` — the seed-lead taken back out | `RED: NoUnmanageableCredential` | 1 872 634 | 219 254 | 13 | 5 s |
| `Historical_E77.cfg` — the grant back in phase 2 **and** the consumer fix out | `RED: NoAccessibleSecretWithoutGate` | 1 634 671 | 191 868 | 13 | 5 s |
| 30 × `Mut_*.cfg` — mutant against the whole invariant set | all 30 RED | 89 – 3 373 706 | 40 – 376 286 | 4 – 14 | 0 s – 8 s |
| 33 × `Solo_*.cfg` — the same mutants against their **own** target only | all 33 RED | 89 – 5 840 454 | 40 – 628 113 | 4 – 15 | 0 s – 13 s |
| 3 × `SoloClause_*.cfg` — and against **one clause** of it | all 3 RED | 24 706 – 20 480 029 | 5 642 – 2 063 820 | 8 – 18 | 1 s – 39 s |
| `Fairness.cfg` — `ENABLED OpAdvances => ~Idle`, at the liveness constants | **GREEN** | 105 147 241 | 10 720 348 | 46 | 157 s |
...
| 28 × `SeamMut_*.cfg` / `SeamSolo_*.cfg` | all 28 RED | 77 – 1 830 | 27 – 211 | 4 – 7 | 0 s – 1 s |
...
| `Lattice.cfg` — the fourth module: the retry/recovery lattice | **GREEN** | 2 431 | 243 | 11 | 0 s |
| 6 × `LatMut_*.cfg` / `LatSolo_*.cfg` | all 6 RED | 2 – 83 | 2 – 33 | 2 – 4 | 0 s – 1 s |
| `Policies.cfg` — all four applets' stateful operation policies | **GREEN** | 45 253 | 2 268 | 14 | 0 s |
| 14 × `PolicyMut_*.cfg` / `PolicySolo_*.cfg` | all 14 RED | 33 – 692 | 14 – 126 | 3 – 5 | 0 s – 1 s |
...
| `Transport.cfg` — the eighth module: the CTAPHID reassembler | **GREEN** | 127 | 13 | 5 | 1 s |
...
| 4 × `TokenRefinement*.cfg` — phase 5: native B -> A state refinement | 2 GREEN, 2 RED | 2 – 34 038 747 | 2 – 3 472 034 | 2 – 44 | 1 s – 208 s |
| `Liveness.cfg` — the three temporal properties, at the heap `floors.txt` gives it | **GREEN** | 105 147 241 | 10 720 348 | 46 | 1782 s |
| 3 × `LiveMut_*.cfg` — one mutant per property | all 3 RED | 491 442 – 604 573 | 73 436 – 94 665 | — | 4 s |
...
configuration. 195 safety rows in **3225 s** and 4 liveness rows in **1794 s**;
...
it is **GREEN over 10 720 348 distinct states in 1782 s**, in the same matrix
