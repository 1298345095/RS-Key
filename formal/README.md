...
| `crates/rsk-fido/src/reset.rs:126` `>` → `>=` | conformance — the runaway valve trips one delete early | recorded |
| `crates/rsk-fido/src/reset.rs:126` `>` → `==` | **the valve stops guarding**: `deleted` rises a whole batch at a time and can step past the threshold without ever equalling it. Not drivable in a unit test at `RESET_MAX_DELETES = 4 × 256 + 15` | **open** |
