...
|---|---|---|---|---|---|
| `Shipped.cfg` — the tree as it stands, `SYMMETRY` on, firmware constants | **GREEN** | 1 401 505 939 | 108 618 956 | 58 | 2853 s |
| `AlwaysUv.cfg` — AS-AUTH-2's other arm: the build that ships it | **GREEN** | 385 191 607 | 31 451 172 | 51 | 741 s |
| `PermWide.cfg` — `WidePerms`'s other arm: all sixteen permission subsets | **GREEN** | 497 332 297 | 29 185 896 | 45 | 796 s |
| 2 × `PermWideMut_*.cfg` — and what says that arm's observers can fail | all 2 RED | 3 967 – 157 120 | 1 285 – 18 193 | 7 – 9 | 1 s |
...
| `Historical_E76.cfg` — the seed-lead taken back out | **GREEN** | 1 401 567 499 | 108 618 956 | 58 | 2918 s |
| `Historical_E77.cfg` — the grant back in phase 2 **and** the consumer fix out | `RED: NoAccessibleSecretWithoutGate` | 2 101 950 | 234 816 | 13 | 5 s |
| 31 × `Mut_*.cfg` — mutant against the whole invariant set | all 31 RED | 89 – 4 676 569 | 40 – 492 641 | 4 – 14 | 0 s – 11 s |
| 34 × `Solo_*.cfg` — the same mutants against their **own** target only | all 34 RED | 89 – 5 758 793 | 40 – 621 211 | 4 – 15 | 0 s – 12 s |
| 3 × `SoloClause_*.cfg` — and against **one clause** of it | all 3 RED | 28 789 – 28 292 464 | 6 437 – 2 706 042 | 8 – 18 | 1 s – 57 s |
| `Fairness.cfg` — `ENABLED OpAdvances => ~Idle`, at the liveness constants | **GREEN** | 151 264 817 | 14 514 424 | 46 | 234 s |
...
| 20 × `PolicyMut_*.cfg` / `PolicySolo_*.cfg` | all 20 RED | 39 – 14 232 | 16 – 2 029 | 3 – 6 | 0 s – 1 s |
...
| 4 × `TokenRefinement*.cfg` — phase 5: native B -> A state refinement | 2 GREEN, 2 RED | 2 – 48 737 609 | 2 – 4 611 484 | 2 – 44 | 1 s – 328 s |
| `Liveness.cfg` — the three temporal properties, at the heap `floors.txt` gives it | **GREEN** | 151 264 817 | 14 514 424 | 46 | 2612 s |
| 3 × `LiveMut_*.cfg` — one mutant per property | all 3 RED | 498 086 – 618 305 | 77 426 – 90 328 | — | 4 s |
...
<!-- Generated by scripts/run_count_gate.py --write; do not edit. -->
**Every row above is from one run on 2026-09-16**: `./run-tlc.sh safety` and
...
configuration. 224 safety rows in **8793 s** and 4 liveness rows in **2624 s**;
...
it is **GREEN over 14 514 424 distinct states in 2612 s**, in the same matrix
