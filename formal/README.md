...
| **Model-blind** | the dynamic-file registry in `scan` (`fs.rs:237` and `fs.rs:240`, three mutants), `try_has_data`'s zero-length test (`fs.rs:331`), `factory_wipe`'s 64-key batch bound (`fs.rs:453`), the registry retain in `delete` (`fs.rs:553`), and **`meta_delete`'s fault guard (`fs.rs:756`)** |
...
sibling `Fs::meta_delete` has the identical guard at `fs.rs:758`, and **nothing
...
`known_absent` and rebuilds the blob from empty (`fs.rs:719`), so the records go
...
`sweep` batches whatever `for_each_key` yields, and `fs.rs:334-337` documents
...
| Mutation switch | Rebuilds | Target invariant | Caught in |
|---|---|---|---|
| `BugDeleteValueBeforeMeta` | `fs.rs:547-549` — the two backend writes reversed, so a torn delete leaves value-gone-meta-alive (`delete_landed`) | `NoOrphanedMetadata` | 54 states |
...
| `BugDeleteHidesFaultedDrop` | the shipped tree before `fs.rs:554` — a faulted `meta_delete` swallowed, so the caller hears `Ok` about a record standing over a value that is gone | `NoSilentOrphan` | 61 states |
...
empty (`fs.rs:719`), losing `b`. This is SEC-STORE-004's damage arriving from a
...
- **The order within a sweep phase is arbitrary.** `for_each_key` yields in
  flash-ring order (`fs.rs:334-337`), which is *a* fixed order per device state,
