...
|---|---|---|---|---|---|
| `Shipped.cfg` — the tree as it stands, `SYMMETRY` on, firmware constants | **GREEN** | 986 836 197 | 77 563 872 | 58 | 1984 s |
| `AlwaysUv.cfg` — AS-AUTH-2's other arm: the build that ships it | **GREEN** | 273 124 081 | 23 521 512 | 51 | 536 s |
| `PermWide.cfg` — `WidePerms`'s other arm: all sixteen permission subsets | **GREEN** | 344 527 413 | 21 456 020 | 45 | 555 s |
| 2 × `PermWideMut_*.cfg` — and what says that arm's observers can fail | all 2 RED | 3 921 – 148 175 | 1 286 – 17 311 | 7 – 9 | 1 s |
| `ForceChange.cfg` — `ForceChangeModelled`'s other arm: EF_MINPINLEN[1], the forced-PIN-change gate | **GREEN** | 106 956 959 | 10 765 176 | 46 | 182 s |
| `Historical_E76.cfg` — the seed-lead taken back out | `RED: NoUnmanageableCredential` | 1 874 409 | 219 578 | 13 | 5 s |
| `Historical_E77.cfg` — the grant back in phase 2 **and** the consumer fix out | `RED: NoAccessibleSecretWithoutGate` | 1 655 404 | 194 797 | 13 | 5 s |
| 31 × `Mut_*.cfg` — mutant against the whole invariant set | all 31 RED | 89 – 3 340 977 | 40 – 371 879 | 4 – 14 | 0 s – 8 s |
| 34 × `Solo_*.cfg` — the same mutants against their **own** target only | all 34 RED | 89 – 5 352 889 | 40 – 583 229 | 4 – 15 | 0 s – 12 s |
| 3 × `SoloClause_*.cfg` — and against **one clause** of it | all 3 RED | 23 504 – 20 480 388 | 5 359 – 2 063 890 | 8 – 18 | 1 s – 40 s |
| `Fairness.cfg` — `ENABLED OpAdvances => ~Idle`, at the liveness constants | **GREEN** | 105 147 241 | 10 720 348 | 46 | 164 s |
...
| 14 × `PolicyMut_*.cfg` / `PolicySolo_*.cfg` | all 14 RED | 33 – 542 | 14 – 107 | 3 – 4 | 0 s – 1 s |
...
| 4 × `TokenRefinement*.cfg` — phase 5: native B -> A state refinement | 2 GREEN, 2 RED | 2 – 34 038 747 | 2 – 3 472 034 | 2 – 44 | 0 s – 217 s |
| `Liveness.cfg` — the three temporal properties, at the heap `floors.txt` gives it | **GREEN** | 105 147 241 | 10 720 348 | 46 | 2020 s |
| 3 × `LiveMut_*.cfg` — one mutant per property | all 3 RED | 471 816 – 599 373 | 71 356 – 92 227 | — | 4 s |
...
configuration. 204 safety rows in **4117 s** and 4 liveness rows in **2032 s**;
24 GREEN, 184 RED, and not one row missed what `floors.txt` requires of it —
