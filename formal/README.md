...
is in use". The code does not: `getassertion.rs:421` and `makecredential.rs:544`
...
|---|---|---|---|---|---|
| `Shipped.cfg` — the tree as it stands, `SYMMETRY` on, firmware constants | **GREEN** | 986 836 197 | 77 563 872 | 58 | 1920 s |
| `AlwaysUv.cfg` — AS-AUTH-2's other arm: the build that ships it | **GREEN** | 273 124 081 | 23 521 512 | 51 | 522 s |
| `PermWide.cfg` — `WidePerms`'s other arm: all sixteen permission subsets | **GREEN** | 344 527 413 | 21 456 020 | 45 | 552 s |
| 2 × `PermWideMut_*.cfg` — and what says that arm's observers can fail | all 2 RED | 3 799 – 142 131 | 1 240 – 16 582 | 7 – 9 | 1 s – 2 s |
...
| `Historical_E76.cfg` — the seed-lead taken back out | **GREEN** | 986 872 269 | 77 563 872 | 58 | 1933 s |
| `Historical_E77.cfg` — the grant back in phase 2 **and** the consumer fix out | `RED: NoAccessibleSecretWithoutGate` | 1 725 880 | 201 529 | 13 | 5 s |
| 31 × `Mut_*.cfg` — mutant against the whole invariant set | all 31 RED | 89 – 3 376 419 | 40 – 377 512 | 4 – 14 | 1 s – 9 s |
| 34 × `Solo_*.cfg` — the same mutants against their **own** target only | all 34 RED | 89 – 5 377 089 | 40 – 586 466 | 4 – 15 | 0 s – 12 s |
| 3 × `SoloClause_*.cfg` — and against **one clause** of it | all 3 RED | 23 276 – 20 479 016 | 5 331 – 2 063 571 | 8 – 18 | 1 s – 41 s |
...
| 20 × `PolicyMut_*.cfg` / `PolicySolo_*.cfg` | all 20 RED | 34 – 14 232 | 14 – 2 033 | 3 – 6 | 0 s – 1 s |
...
| 2 × `Historical_Boot*.cfg` — the write/re-arm order the tree ships, and the one it does not | 1 GREEN, 1 RED | 25 – 141 | 14 – 38 | 4 – 7 | 1 s |
...
| `Liveness.cfg` — the three temporal properties, at the heap `floors.txt` gives it | **GREEN** | 105 147 241 | 10 720 348 | 46 | 2103 s |
| 3 × `LiveMut_*.cfg` — one mutant per property | all 3 RED | 497 816 – 611 615 | 73 890 – 95 702 | — | 4 s – 5 s |
...
<!-- Generated by scripts/run_count_gate.py --write; do not edit. -->
**Every row above is from one run on 2026-09-08**: `./run-tlc.sh safety` and
...
configuration. 224 safety rows in **5994 s** and 4 liveness rows in **2116 s**;
...
- `ConfirmNamesTheOperation` — an operation that names a relying party
  ...
  `getassertion.rs:652-653`, `makecredential.rs:688-689`, `u2f.rs:94`);
...
it is **GREEN over 10 720 348 distinct states in 2103 s**, in the same matrix
