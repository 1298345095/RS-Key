...
|---|---|---|---|---|---|
| `Shipped.cfg` — the tree as it stands, `SYMMETRY` on, firmware constants | **GREEN** | 986 836 197 | 77 563 872 | 58 | 1913 s |
| `AlwaysUv.cfg` — AS-AUTH-2's other arm: the build that ships it | **GREEN** | 273 124 081 | 23 521 512 | 51 | 519 s |
| `PermWide.cfg` — `WidePerms`'s other arm: all sixteen permission subsets | **GREEN** | 344 527 413 | 21 456 020 | 45 | 553 s |
| `ForceChange.cfg` — `ForceChangeModelled`'s other arm: EF_MINPINLEN[1], the forced-PIN-change gate | **GREEN** | 106 956 959 | 10 765 176 | 46 | 177 s |
| `Historical_E76.cfg` — the seed-lead taken back out | `RED: NoUnmanageableCredential` | 1 875 109 | 219 667 | 13 | 5 s |
| `Historical_E77.cfg` — the grant back in phase 2 **and** the consumer fix out | `RED: NoAccessibleSecretWithoutGate` | 1 665 877 | 196 340 | 13 | 4 s |
| 31 × `Mut_*.cfg` — mutant against the whole invariant set | all 31 RED | 89 – 3 350 977 | 39 – 373 970 | 4 – 14 | 1 s – 8 s |
| 34 × `Solo_*.cfg` — the same mutants against their **own** target only | all 34 RED | 89 – 5 521 139 | 40 – 599 634 | 4 – 15 | 0 s – 12 s |
| 3 × `SoloClause_*.cfg` — and against **one clause** of it | all 3 RED | 23 358 – 20 768 430 | 5 350 – 2 096 258 | 8 – 18 | 1 s – 41 s |
| `Fairness.cfg` — `ENABLED OpAdvances => ~Idle`, at the liveness constants | **GREEN** | 105 147 241 | 10 720 348 | 46 | 159 s |
...
| 28 × `SeamMut_*.cfg` / `SeamSolo_*.cfg` | all 28 RED | 77 – 1 902 | 27 – 213 | 4 – 7 | 0 s – 1 s |
...
| 14 × `PolicyMut_*.cfg` / `PolicySolo_*.cfg` | all 14 RED | 33 – 679 | 14 – 126 | 3 – 5 | 0 s – 1 s |
...
| 4 × `TokenRefinement*.cfg` — phase 5: native B -> A state refinement | 2 GREEN, 2 RED | 2 – 34 038 747 | 2 – 3 472 034 | 2 – 44 | 1 s – 217 s |
| `Liveness.cfg` — the three temporal properties, at the heap `floors.txt` gives it | **GREEN** | 105 147 241 | 10 720 348 | 46 | 2045 s |
| 3 × `LiveMut_*.cfg` — one mutant per property | all 3 RED | 499 898 – 585 854 | 74 168 – 92 530 | — | 4 s |
...
<!-- Generated by scripts/run_count_gate.py --write; do not edit. -->
**Every row above is from one run on 2026-08-30**: `./run-tlc.sh safety` and
...
configuration. 202 safety rows in **4020 s** and 4 liveness rows in **2057 s**;
24 GREEN, 182 RED, and not one row missed what `floors.txt` requires of it —
...
The live roster is **73 entries: 68 executable patches killed, five unreachable
...
it is **GREEN over 10 720 348 distinct states in 2045 s**, in the same matrix
