...
|---|---|---|---|---|---|
| `Shipped.cfg` — the tree as it stands, `SYMMETRY` on, firmware constants | **GREEN** | 986 836 197 | 77 563 872 | 58 | 1979 s |
| `AlwaysUv.cfg` — AS-AUTH-2's other arm: the build that ships it | **GREEN** | 273 124 081 | 23 521 512 | 51 | 548 s |
| `PermWide.cfg` — `WidePerms`'s other arm: all sixteen permission subsets | **GREEN** | 344 527 413 | 21 456 020 | 45 | 579 s |
| 2 × `PermWideMut_*.cfg` — and what says that arm's observers can fail | all 2 RED | 3 874 – 129 253 | 1 278 – 15 290 | 7 – 9 | 1 s |
| `ForceChange.cfg` — `ForceChangeModelled`'s other arm: EF_MINPINLEN[1], the forced-PIN-change gate | **GREEN** | 106 956 959 | 10 765 176 | 46 | 186 s |
| `Historical_E76.cfg` — the seed-lead taken back out | **GREEN** | 986 872 269 | 77 563 872 | 58 | 1950 s |
| `Historical_E77.cfg` — the grant back in phase 2 **and** the consumer fix out | `RED: NoAccessibleSecretWithoutGate` | 1 689 037 | 198 290 | 13 | 5 s |
| 31 × `Mut_*.cfg` — mutant against the whole invariant set | all 31 RED | 89 – 3 378 829 | 40 – 377 854 | 4 – 14 | 0 s – 8 s |
| 34 × `Solo_*.cfg` — the same mutants against their **own** target only | all 34 RED | 89 – 5 502 199 | 40 – 598 074 | 4 – 15 | 0 s – 12 s |
| 3 × `SoloClause_*.cfg` — and against **one clause** of it | all 3 RED | 24 799 – 20 472 181 | 5 653 – 2 062 384 | 8 – 18 | 1 s – 40 s |
...
| 28 × `SeamMut_*.cfg` / `SeamSolo_*.cfg` | all 28 RED | 77 – 1 762 | 27 – 207 | 4 – 7 | 0 s – 1 s |
...
| 20 × `PolicyMut_*.cfg` / `PolicySolo_*.cfg` | all 20 RED | 39 – 14 400 | 16 – 2 057 | 3 – 6 | 0 s – 1 s |
...
| 4 × `TokenRefinement*.cfg` — phase 5: native B -> A state refinement | 2 GREEN, 2 RED | 2 – 34 038 747 | 2 – 3 472 034 | 2 – 44 | 1 s – 216 s |
| `Liveness.cfg` — the three temporal properties, at the heap `floors.txt` gives it | **GREEN** | 105 147 241 | 10 720 348 | 46 | 2029 s |
| 3 × `LiveMut_*.cfg` — one mutant per property | all 3 RED | 506 982 – 619 628 | 74 697 – 97 271 | — | 4 s |
...
<!-- Generated by scripts/run_count_gate.py --write; do not edit. -->
**Every row above is from one run on 2026-08-31**: `./run-tlc.sh safety` and
...
configuration. 210 safety rows in **6113 s** and 4 liveness rows in **2041 s**;
...
it is **GREEN over 10 720 348 distinct states in 2029 s**, in the same matrix
