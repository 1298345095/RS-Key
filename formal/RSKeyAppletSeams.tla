...
\* (crates/rsk-oath/src/lib.rs:524-526) -- measured on a YubiKey 5.7.4 from a
...
\* rather than OpenPGP's (crates/rsk-fido/src/reset.rs:219-261). Modelled as a
\* step that changes nothing, so a mutant that made it reach would be visible.
FidoReset == UNCHANGED vars

\* `Fs::factory_wipe` (crates/rsk-fs/src/fs.rs:400-447) is FLASH-only: it never
...
\* the standing unlock (crates/rsk-oath/src/lib.rs:524-526), because a MAC
\* challenge-response has no retry counter for a refusal to protect.
\*
