...
CONSTANTS
    ...
    Fids,
    \* crates/rsk-fs/src/fs.rs:494-514 -- `Fs::delete` drops the metadata FIRST,
    ...
    \* unconditional now (fs.rs:521). The switch gates it on the value again.
    BugDeleteMetaOnlyUnderPresent,
    \* fs.rs:521-528 -- a FAILED EF_META read cannot drop the record, and the
    ...
    \* audit run-36: `settle` (fs.rs:192-198) refuses to cache a
    ...
    \* fs.rs:253-255 -- `scan` fills the decided bitmap for the WHOLE FID space
    ...
    \* The 0x077C databug's meta half: `meta_add_reserve` (fs.rs:695-697) treats
    ...
    \* fs.rs:727 -- `meta_delete` refuses a FAILED EF_META read (MemoryFatal)
...
VARIABLES
    ...
    \* (`known_absent`, fs.rs:159-161); an undecided FID falls through to the
...
\* `Fs::put` (fs.rs:460-480): write the value, then `mark_present` -- which
\* sets BOTH the present and the decided bit (fs.rs:175-179).
Put(f, v) ==
    /\ val'     = [val     EXCEPT ![f] = v]
...
\* `Fs::meta_add_reserve` (fs.rs:682-708): rewrite EF_META with `fid`'s record
...
MetaAdd(f) ==
    ...
    \* blob is rebuilt from empty (fs.rs:688-690). Correct while the cache is
...
\* `Fs::meta_delete` (fs.rs:718-748): drop `fid`'s record, and clear EF_META
\* once the last one goes. A FAILED read of EF_META must refuse (fs.rs:727) --
...
(* writes: metadata FIRST, then the value (fs.rs:521-528). `k` is how many   *)
...
\* answer through `record_unless_faulted` (fs.rs:263-292). `fault` is whether
...
\* `Fs::scan` (fs.rs:213-256): clear the caches, then walk the backend. `seen`
...
\* tree can be in -- what it may not do is REPORT SUCCESS from it (fs.rs:528).
...
\* is: `meta_add` TRUSTS this bit and rebuilds the blob from empty (fs.rs:688),
