...
CONSTANTS
    ...
    \* The 0x077C databug's meta half: `meta_add_reserve` (fs.rs:700-702) treats
    ...
    \* fs.rs:732 -- `meta_delete` refuses a FAILED EF_META read (MemoryFatal)
...
\* `Fs::meta_add_reserve` (fs.rs:687-713): rewrite EF_META with `fid`'s record
...
MetaAdd(f) ==
    ...
    \* blob is rebuilt from empty (fs.rs:693-695). Correct while the cache is
...
\* `Fs::meta_delete` (fs.rs:723-753): drop `fid`'s record, and clear EF_META
\* once the last one goes. A FAILED read of EF_META must refuse (fs.rs:732) --
...
\* is: `meta_add` TRUSTS this bit and rebuilds the blob from empty (fs.rs:693),
