...
CONSTANTS
    ...
    Fids,
    \* crates/rsk-fs/src/fs.rs:447-467 -- `Fs::delete` drops the metadata FIRST,
    ...
    \* unconditional now (fs.rs:474). The switch gates it on the value again.
    BugDeleteMetaOnlyUnderPresent,
    \* fs.rs:474-481 -- a FAILED EF_META read cannot drop the record, and the
    ...
    \* audit run-36: `record_unless_faulted` (fs.rs:179-183) refuses to cache a
    ...
    \* fs.rs:238-240 -- `scan` fills the decided bitmap for the WHOLE FID space
    ...
    \* The 0x077C databug's meta half: `meta_add_reserve` (fs.rs:631-633) treats
    ...
    \* fs.rs:663 -- `meta_delete` refuses a FAILED EF_META read (MemoryFatal)
...
VARIABLES
    ...
    present,  \* [Fids -> BOOLEAN]: the in-RAM present-cache bit (fs.rs:79)
    \* [Fids -> BOOLEAN]: the authority bit paired with `present` (fs.rs:88). A
    \* clear `present` is trusted as absent ONLY once `decided` confirms it
    \* (`known_absent`, fs.rs:150-152); an undecided FID falls through to the
...
\* `Fs::put` (fs.rs:413-433): write the value, then `mark_present` -- which
\* sets BOTH the present and the decided bit (fs.rs:166-170).
Put(f, v) ==
    /\ val'     = [val     EXCEPT ![f] = v]
...
\* `Fs::meta_add_reserve` (fs.rs:618-644): rewrite EF_META with `fid`'s record
...
MetaAdd(f) ==
    ...
    \* blob is rebuilt from empty (fs.rs:624-626). Correct while the cache is
...
\* `Fs::meta_delete` (fs.rs:654-684): drop `fid`'s record, and clear EF_META
\* once the last one goes. A FAILED read of EF_META must refuse (fs.rs:663) --
...
(* writes: metadata FIRST, then the value (fs.rs:474-481). `k` is how many   *)
...
\* answer through `record_unless_faulted` (fs.rs:244-273). `fault` is whether
...
\* `Fs::scan` (fs.rs:198-241): clear the caches, then walk the backend. `seen`
...
\* tree can be in -- what it may not do is REPORT SUCCESS from it (fs.rs:481).
...
\* is: `meta_add` TRUSTS this bit and rebuilds the blob from empty (fs.rs:624),
