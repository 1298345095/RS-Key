...
CONSTANTS
    BugResetGatesFirst,           \* reset.rs:89-90   two-phase wipe order
    ...
    BugWarmResetReopensWindow,    \* reset.rs:246-247  in_reset_window
    BugCmWalkIgnoresChannel,      \* state.rs:169-180  may_walk_rps
    BugDeleteRpBeforeCred,        \* credmgmt.rs:665-673 deleteCredential order
    BugBackupSealedNotAGate,      \* reset.rs:202-239 is_fido_gate_fid (run-36)
    ...
    BugSeedDoesNotLead,           \* reset.rs:74-86 / fs.rs `first`, pre-0x08BF
    BugNoTouchRequired,           \* the presence gate on mc / ga
    BugStateResetAfterWipe,       \* reset.rs:70-73 ctx.state.reset() ordering
...
VARIABLES
    ...
    \* the rest: its ABSENCE is the permissive state (reset.rs:202-239), so what
    \* a torn wipe can re-open is a window the owner had closed.
    gate,   \*                                                 (reset.rs:197-240)
...
(* (reset.rs:160-164), and `Ctx::load_keydev` reads it from RAM first and   *)
...
(***************************************************************************)
(* authenticatorReset -- reset.rs:36-110. Two phases, each a batch of        *)
...
\* reset.rs:242-247. A warm boot CLOSES the window rather than opening one:
...
\*
\* THE LIVE SESSION GOES FIRST, ahead of every flash write (reset.rs:70-73). That
...
\* (reset.rs:202-239): it is in the GATE set, so the marker outlives the seed it
...
\* reset.rs:74-86 -- the seed goes in its own force_delete AHEAD of the batch, so
...
\* Phase 1, reset.rs:89 -- every live FIDO-owned fid that is NOT a gate. One
...
\* Phase 2, reset.rs:90 -- the records that GATE the applet rather than being
    ...
    /\ IF GatesLive
         THEN /\ \/ (pin.set /\ pin' = PinRecordDeleted
                 ...
                 \/ (SealedIsAGate /\ gate' = [gate EXCEPT !.backupSealed = FALSE]
                                    /\ UNCHANGED pin)
                 \* reset.rs:238 puts EF_MINPINLEN in this phase, and :207-214
...
\* reset.rs:99: ensure_seed. The session already died at reset.rs:73, ahead of the
...
\* Any `?` in reset.rs:82-99 -- a force_delete that errors, a truncated
...
\* PinLock across it; reset.rs:247 makes it CLOSE the reset window.
WarmReset ==
    /\ VolatileCleared
...
\* surviving usable secret whose gate has already gone (reset.rs:64-73).
