...
Read out of `target/thumbv8m.main-none-eabihf/release/firmware` with `arm-none-eabi-objdump -d -l --inlines`, built by rustc 1.96.0 (ac68faa20). A site's verdict is `constant-time` when no conditional branch inside any address run the inline chain attributes to it reads flags set from a buffer load; the callers are the first-party frames those chains name, so a surface that stops routing through the site leaves this table.

| Site | Class | Verdict | Surfaces that inline it |
|---|---|---|---|
| `rsk_crypto::mac::ct_eq` | comparator | constant-time | CTAP pinUvAuthProtocol MAC verify, FIDO device-local PIN verify, FIDO clientPIN hash verify, FIDO credential key-handle verify, vendor device-PIN gate, OATH SET CODE, OATH VALIDATE, OATH PIN-gated code match, OpenPGP default reset-code neutralisation, OpenPGP PW1/PW3/RC verify, OTP slot configure/update access code, OTP access-code sweep, PIV slot metadata default-value check, PIV mutual authenticate, PIV single authenticate, PIV PIN/PUK reference verify |
