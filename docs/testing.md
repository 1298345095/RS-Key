...
`safety` is the nine shipped models, the 85 mutation switches that have a
configuration family of their own (87 `Bug*` switches exist;
...
<!-- Generated by scripts/run_count_gate.py --write; do not edit. -->
Both tiers are measured runs, not sums. On 2026-09-02, on the Apple M5 Pro (18
cores) of the Kani table above at the default `WORKERS=2`, `safety` came back
over **214 configurations in 6446 s — 25 GREEN, 189 RED, and not one row that
missed what `floors.txt` asks of it**; `liveness` took **2083 s** for its 4, at
...
<!-- Generated by scripts/run_count_gate.py --write; do not edit. -->
The original phase-2 baseline is fixed at 31 rows, and the full 80-entry live
roster runs weekly.
