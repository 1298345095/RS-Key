...
| `SEC-DISP-001` | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | cov | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos |
| `SEC-DISP-002` | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | cov | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos |
| `SEC-DISP-003` | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | cov | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos | oos |
...
**covered** — basis `stated`

- columns: `firmware-display`
- properties: `SEC-DISP-001`, `SEC-DISP-002`, `SEC-DISP-003`
- The only column that compiles the ceremony, and the one the display evidence was produced on: `formal/Display.cfg`, the `rsk-ui`/`rsk-display` host tests, and the five `check.sh` rows that compile the feature (`clippy (display firmware)`, `clippy (display strong-pin)`, `clippy (display wiring)`, `test (display wiring)`, `build firmware (display)`).
...
| `firmware-always-uv` | 37 | `always-uv` ships `alwaysUv` ON, which changes what `SEC-FIDO-001`'s gate DEMANDS rather than whether it is enforced — and `SEC-FIDO-006B` is about the alwaysUv gate surviving a reset, with a compiled-in default this column moves. Do the model's gate constants cover the alwaysUv-on arm? |
...
| `waveshare-touch-lcd` | 31 | The trusted-display BOARD without the `display` feature: 16 MB geometry, no addressable LED, and panel pins compiled as inert constants. The geometry question is `firmware-16mb`'s; the sharper one is whether this preset is ever built WITHOUT `--features display`, and if not, why it is a column of its own rather than a knob of `firmware-display`. |
