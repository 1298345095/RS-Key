...
| `assurance/threat_clauses.toml` + `scripts/threat_gate.py` | the root: `SEC-FIDO-001`'s `source` names a clause of the [threat model](threat-model.md), `TM-HOST-GATES`, and 33 of 40 P0-family properties are traced across 45 clauses (32 defence, 13 context), 7 untraced | say anything about *sufficiency* — a clause is an anchor, not an argument that the property covers it |
