...
80 entries: all 75 executable patches are killed and five are unreachable.
...
Both timings are one run each on 2026-09-01, on an Apple M5 Pro (18 cores) at
the default `WORKERS=2`: `safety` over 210 configurations in 6189 s and
`liveness` over 4 in 2319 s — 25 GREEN, 189 RED, no row short of its floor.
