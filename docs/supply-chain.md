...
on every `check.sh` run, so the sentences on this page cannot describe a
pipeline the repository has stopped running.
...
The commands those steps run, as this gate reads them: `\` continuations joined, and everything from the first ` #` on a line dropped — which also truncates a line whose *string* holds one.
