...
//! * that `scan` registers every file it is handed. `fs.rs:217` skips
...
/// never rebuilt from an empty blob — `meta_add_reserve`'s guard, `fs.rs:631-633`.
///
...
/// becomes a decided absence — `meta_delete`'s guard, `fs.rs:663`, and the one thing
