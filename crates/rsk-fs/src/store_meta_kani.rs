...
//! * that `scan` registers every file it is handed. `fs.rs:228` skips
...
/// never rebuilt from an empty blob — `meta_add_reserve`'s guard, `fs.rs:714-716`.
///
...
/// becomes a decided absence — `meta_delete`'s guard, `fs.rs:746`, and the one thing
