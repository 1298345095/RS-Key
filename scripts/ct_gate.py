...
#: and would fail it. That is why it is confined to the paragraph that carries
#: the anchor, and why a missing anchor is itself a finding: a rule keyed on a
#: string the page can simply drop is a rule the next edit deletes for free.
SCOPE_ANCHOR = "Its scope is"
SCOPE_REFUTED = re.compile(r"\bkeygen\b")
...
#: The fourth floor, and it answers a hole the three above cannot see.
#: `reload_of_a_store` is the only rule here that EXCUSES a load, and none of the
#: other counts depends on it: runs, branches and traced are all computed without
#: asking it. So a version of it that answers True too often is invisible.
#: Driven, not argued — stubbed to `True` over the shipped image the row reports
#: 0 secret-dependent branches with 39 / 25 / 22 unchanged and EXIT=0, and the
#: table's own early-exit fixture loses its finding as well. A check that cannot
#: fail, wearing green, which is the shape this file exists to refuse.
#:
#: Measured 2026-09-01 on the default release image: 82 loads inside the site's
#: runs, 28 excused as a reload and 54 still visible to the taint. 43 is the same
#: ~20% under the measurement `RUN_FLOOR` and `BRANCH_FLOOR` carry. A `pc`-based
#: literal load counts as neither, because `buffer_load` refuses it before the
#: excuse is ever put — there are none inside the runs today, and writing it the
#: other way would credit the excuse for a load it never decided.
EXPOSED_FLOOR = 43
...
def excused_loads(stream, symbol):
    """(reloads excused through a base other than `sp`, loads still exposed).

    The first is the count the docstring used to assert and now prints; the
    second is what `EXPOSED_FLOOR` is on. Both come from the same question
    `buffer_load` puts, in the same order — `pc` first, then the excuse — so a
    load this counts as exposed is a load the taint can still reach.
    """
    spilled = exposed = 0
    for index, (_, mnemonic, operands, chain) in enumerate(stream):
        ...
        if not reload_of_a_store(stream, index):
            exposed += 1
        elif base and base.group(1) != "sp":
            spilled += 1
    return spilled, exposed


def scope_finding(text: str) -> str | None:
    """What `docs/ct-audit.md`'s scope paragraph may not say, or None."""
    scope = next(
        (block for block in text.split("\n\n") if SCOPE_ANCHOR in block), None
    )
    if scope is None:
        return (
            f"{PAGE}: no paragraph says `{SCOPE_ANCHOR}` — the page's scope is the"
            " only prose here anything holds, and dropping the sentence drops the"
            " rule with it"
        )
...
def observe(root: pathlib.Path, sites, lines=None, loads=None):
    ...
    `loads` collects `excused_loads`'s two totals, the same way `asked` collects
    the branches the rule reasoned about: the stream is built here, and building
    it a second time in `audit` would double a 15 s row.
    """
    ...
    for name, entry in sorted(sites.items()):
        ...
        if loads is not None:
            spilled, exposed = excused_loads(stream, entry["symbol"])
            loads["spilled"] = loads.get("spilled", 0) + spilled
            loads["exposed"] = loads.get("exposed", 0) + exposed
...
def audit(
    ...
    # And the floor none of the three above can stand in for: they are all
    # computed WITHOUT asking `reload_of_a_store`, so an excuse that widened
    # until it covers every load leaves them untouched and the row green over a
    # rule that decides nothing.
    if loads.get("exposed", 0) < exposed_floor:
        findings.append(
            f"{loads.get('exposed', 0)} in-site load(s) still visible to the"
            f" taint, under the measured {exposed_floor} — the reload excuse"
            " widened, and no other count here moves when it does"
        )
    ...
    summary = (
        ...
        f" and {total_reasoned} traced to a definition, {loads.get('exposed', 0)}"
        f" load(s) still exposed to the taint and {loads.get('spilled', 0)}"
